...
Your query should return the film title, description, 
...
the customer's first name, last name, email, and address.*/
...
special features, genre (category), and actor's first and last name.*/
select film.title, film.description, film.release_year, film.rating, 
film.special_features, category.name, actor.first_name, actor.last_name
from film
join film_category on film.film_id = film_category.film_id
join category on category.category_id = film_category.category_id
join film_actor on film.film_id = film_actor.film_id
